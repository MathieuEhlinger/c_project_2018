...
def cells_id_bird(df, identifiers):
    
    col_list = pd.DataFrame(index = df.index)
    for i in identifiers:
        col_list[i] = df[i]
    return col_list.drop_duplicates().values.tolist()
...
def data_in_dict(df, sheet_identifier='Data'):
...
def transformer(file, directory, size=50, rand=23):
...
#    first = 0
    ...
    datas=data_in_dict(xl)
#    stat_sheet=xl.parse('24h Stat')[:-4]
    writer = pd.ExcelWriter('./'+directory+'/'+file[:-5]+'_bdd'+' .xlsx')
    for i in xl.sheet_names:
        xl.parse(i).to_excel(writer,i, engine='openpyxl', index=False)
    
    for data in datas.keys():
#        size=adapt_sizes_to_df(datas[data].shape[0], size)
        size = [datas[data].shape[0]]
        print('"Sizes" argument have no influence on Bird behaviors')
    #    writer = pd.ExcelWriter('./'+directory+'/'+file[:-5]+' '+ str(selected_size)+' .xlsx')
        

        for selected_size in size :    
            cell_df = pd.DataFrame()
            for ID in cells_id_bird(datas[data], IDs):
                cell_data = datas[data].loc[(datas[data][IDs[0]] == ID[0]) & (datas[data][IDs[1]] == ID[1]) & (datas[data][IDs[2]] == ID[2])].reset_index(drop=True)
    ...
    #            print(cell_data)
        ...
        cell_df.to_excel(writer, (str(selected_size)+' '+data+' -PG'), index=False)
        print(str(selected_size)+' '+data+' -PG')
    writer.save()
...
def main_mom(file_processor = transformer, directory = 'output Bird', size=50, rand=23):
